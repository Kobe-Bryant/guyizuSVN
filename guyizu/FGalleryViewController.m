...
//            UIBarButtonItem *btn = [[[UIBarButtonItem alloc] initWithTitle:@"全部图片" style:UIBarButtonItemStylePlain target:self action:@selector(handleSeeAllTouch:)] autorelease];
...
//    mApplication.window.backgroundColor = [UIColor blackColor];
//    mApplication.leveyTabBarController.tabBar.hidden = YES;
...
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
    ...
    AppDelegate *mApplication = (AppDelegate *)[UIApplication sharedApplication].delegate;
    mApplication.window.backgroundColor = [UIColor colorWithRed:0/255.0 green:160/255.0 blue:211/255.0 alpha:1.0];
    mApplication.leveyTabBarController.tabBar.hidden = NO;
...
- (void)updateTitle
{
	[self setTitle:[NSString stringWithFormat:@"%i / %i", _currentIndex+1, [_photoSource numberOfPhotosForPhotoGallery:self]]];
}
